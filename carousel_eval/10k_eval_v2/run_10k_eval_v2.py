"""
Evaluate V2 carousels from the 10k user CSV format.

Reuses the same evaluation pipeline as the prompt-4 10k eval.
...
Usage:
    python carousel_eval/10k_eval_v2/run_10k_eval_v2.py \
        --input carousel_eval/10k_eval_v2/10k_v2_carousels.csv \
        --orders_csv carousel_eval/10k_eval_v2/10k_v2_orders.csv \
...
# Add parent dirs so we can import shared evaluation code
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Run eval on V2 10k user carousel CSV",
    )
    p.add_argument(
        "--input", required=True,
    ...
    p.add_argument("--source_name", default="v2")
